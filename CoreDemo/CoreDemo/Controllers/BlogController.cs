...
namespace CoreDemo.Controllers
{
	...
	public class BlogController : Controller
	{
		...
		[HttpPost]
		public IActionResult EditBlog(Blog p, IFormFile BlogImage, int BlogId)
		{
            var blogToUpdate = bm.TGetById(BlogId);

            if (blogToUpdate != null)
            {
                blogToUpdate.BlogTitle = p.BlogTitle;
                blogToUpdate.CategoryID = p.CategoryID;
                blogToUpdate.BlogContent = p.BlogContent;
				blogToUpdate.BlogStatus = true;
                blogToUpdate.BlogCreateDate = DateTime.Parse(DateTime.Now.ToShortDateString());
                ...
                if (BlogImage != null && BlogImage.Length > 0)
                {
                    ...
                    // veritabanına kaydetme
                    blogToUpdate.BlogImage = "/images/" + newFileName;
                }

                bm.TUpdate(blogToUpdate);
            }
